...
class Crawler():
    def __init__(self):
        self.urls = [
            "http://cwwp2.dot.ca.gov/vm/loc/d3/hwy50atsierraeb.htm"
        ]
        self.width = 300
        self.height = 500
...
class Screenshot(threading.Thread):
    ...
    def run(self):
        ...
        while (True):
            ...
            name = "data/video/{7},{0}-{1}-{2},{3}-{4}-{5}-{6}.png".format(t.year, t.month, t.day, t.hour, t.minute, t.second, "%.06f" % t.microsecond, "test")
            img = self.get_image()
    ...
    def initialize_browser(self, blocking = False):
        ...
        time.sleep(2.0)
        self.driver.execute_script("""document.getElementById("cctv-Container").setAttribute("style", "width:320px;height:210px");""")
        self.name = self.driver.find_element_by_id("cctv-TitleContent").get_attribute("innerHTML")
        self.format_name()
        print(self.name)
        self.driver.find_element_by_id("cctv-Container").click()
